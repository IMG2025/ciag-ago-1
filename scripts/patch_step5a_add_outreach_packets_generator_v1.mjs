#!/usr/bin/env node
/**
 * Step 5A — Add Outreach Packet Generator (v1)
 * - Reads CIAG Tier-1 outreach queue artifacts produced in Step 4:
 *   out/reach/queue.jsonl (preferred) or out/reach/queue.csv (fallback)
 * - Emits deterministic operator packets to:
 *   out/reach/packs/*.md + out/reach/packs/index.csv
 * - No changes to ICP/scoring/tier logic. Read-only consumption only.
...
const generatorPath = path.join(SCRIPTS, "generate_outreach_packets_from_tier1_v1.mjs");
...
// --- 2) Add npm script safely (if package.json is JSON)
const pkgPath = path.join(ROOT, "package.json");
if (exists(pkgPath)) {
  try {
    const pkg = JSON.parse(read(pkgPath));
    pkg.scripts = pkg.scripts || {};
    if (!pkg.scripts["ciag:packs"]) pkg.scripts["ciag:packs"] = "node scripts/generate_outreach_packets_from_tier1_v1.mjs";
    writeIfChanged(pkgPath, JSON.stringify(pkg, null, 2) + "\n");
  } catch {
    // do not mutate if non-JSON
  }
}
