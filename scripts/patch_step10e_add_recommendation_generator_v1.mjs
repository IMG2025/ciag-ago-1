#!/usr/bin/env node
/**
 * Step 10E — Add policy-aware recommendation generator
 * - Adds: scripts/generate_recommendation_from_risk_register_v1.mjs
 * - Adds: npm script "ciag:recommendation"
...
const GEN = path.join("scripts", "generate_recommendation_from_risk_register_v1.mjs");
...
writeIfChanged(GEN, generatorSrc);
chmod755(GEN);

// package.json: add npm script
const pkg = JSON.parse(read(PKG));
pkg.scripts = pkg.scripts || {};
if (!pkg.scripts["ciag:recommendation"]) {
  pkg.scripts["ciag:recommendation"] = "node scripts/generate_recommendation_from_risk_register_v1.mjs";
