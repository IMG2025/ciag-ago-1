#!/usr/bin/env node
/**
 * patch_wire_core_mcp_phase4_ciag_v1.mjs
 * Idempotent patch:
 * - Adds ago-1-core as dependency
 * - Adds compile-time proof that CIAG can construct an MCP call via core
 * - Runs npm install
...
function main() {
  console.log("== CIAG Phase 4A: wire MCP from ago-1-core ==");
  ...
  // Use GitHub repo directly. Keep master to match what we pushed.
...
`;
  if (writeIfChanged(p("src", "mcp_phase4a_compile_proof.ts"), proofTs)) {
    changed.push("src/mcp_phase4a_compile_proof.ts");
  }
